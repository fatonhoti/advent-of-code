...
def transform_hand(hand):
    ...
    def get_best_hand(original_hand, replacements_dict, current_best, current_hand):
        if not original_hand:
            if len(current_best) == 0:
                current_best.append(current_hand)
            else:
                current_best[0] = current_hand if hand_compare([current_hand], [current_best[0]], card_to_score, False) == -1 else current_best[0]
        else:
            first_char = original_hand[0]
            remaining_str = original_hand[1:]
            if first_char in replacements_dict:
                ...
                for replacement in replacements_dict[first_char]:
                    get_best_hand(remaining_str, replacements_dict,
                                    current_best, current_hand + replacement)
            else:
                # The card was not a joker, add back the original card to the hand
                get_best_hand(remaining_str, replacements_dict,
                                current_best, current_hand + first_char)
    
    best_reworked_hand = []
    get_best_hand(hand, {"J": ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2"]}, best_reworked_hand, "")
